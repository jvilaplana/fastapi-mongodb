...
# However, in Pydantic, field names that start with an underscore are treated as private attributes and cannot be assigned values directly.
# To work around this, we can name the field id in the Pydantic model, but given an alias of _id so it maps correctly to MongoDB.
PyObjectId = Annotated[str, BeforeValidator(str)]


# Primary model used as the response model for the majority of the endpoints.
class BookModel(BaseModel):
    ...
    # The id field is of type PyObjectId, a custom type annotated with a BeforeValidator that coerces the value into a string. This ensures that MongoDB's ObjectId can be accepted and serialized properly by the model.
    ...
    model_config = ConfigDict(
        # Allows the model to be initialized using either the field name (id) or its alias (_id)
        populate_by_name=True,          # https://docs.pydantic.dev/2.0/usage/model_config/#populate-by-name
        # Enables support for custom types such as PyObjectId
        arbitrary_types_allowed=True,   # https://docs.pydantic.dev/2.0/usage/model_config/#arbitrary-types-allowed,
        # Define example data used in FastAPI's autogenerated documentation.
        json_schema_extra={
            "example": {
...
# The UpdateBookModel has the following key differences from the BookModel:
...
# FastAPI encodes and decodes data as JSON strings, which do not support all the data types that MongoDB's BSON data type can store. 
# BSON has support for more non-JSON-native data types, including ObjectId which is used for the default UUID attribute, _id.
# Because of this, you must convert ObjectId objects to strings before storing them in the _id field. The json_encoders setting instructs Pydantic to do this.
# For more information about how BSON compares to JSON, see the JSON and BSON MongoDB article: https://www.mongodb.com/json-and-bson
...
# This class is defined to encapsulate a list of BookModel instances.
# The purpose of this endpoint is to provide some protection from JSON Hijacking, where a malicious user attempts to access sensitive data by exploiting how browsers handle JSON arrays.
# For more information, you can read the JSON Hijacking article: https://haacked.com/archive/2009/06/25/json-hijacking.aspx/
...
# Method to test a successful MongoDB Atlas connection.
@app.get(
    "/test-db-connection/",
